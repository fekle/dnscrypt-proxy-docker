...
# install environment helper script
ARG TARGETPLATFORM
COPY ./envhelper.sh /usr/local/bin/envhelper

# get dnscrypt binary from github releases (see https://github.com/DNSCrypt/dnscrypt-proxy/releases)
ARG DNSCRYPT_PROXY_VERSION
RUN wget -O- -q "https://github.com/DNSCrypt/dnscrypt-proxy/releases/download/${DNSCRYPT_PROXY_VERSION}/dnscrypt-proxy-$(envhelper DNSCRYPT_PROXY_ARCH)-${DNSCRYPT_PROXY_VERSION}.tar.gz" \
    | tar x -z -f- -C /tmp && mv /tmp/* /tmp/dnscrypt-proxy

# install simple-blacklist binary
ARG SIMPLE_BLACKLIST_VERSION
RUN wget -O- "https://github.com/fekle/simple-blacklist/releases/download/${SIMPLE_BLACKLIST_VERSION}/simple-blacklist-$(envhelper SIMPLE_BLACKLIST_ARCH).gz" | \
